...
test_align.idx_equal_duration <- function() {
    ...
    checkEquals(align.idx(t1, t2, sopen=FALSE, eopen=FALSE), 1:10 * 10)
}
...
test_align.idx_after_duration <- function() {
    ...
    t1 <- nanotime(1:100 * one_second_duration * 2 + one_second_duration)
    t2 <- nanotime(1:10 * one_second_duration * 10)
    checkEquals(align.idx(t1, t2, end=one_second_duration, eopen=FALSE), seq(5, 50, 5))
}
...
test_align.idx_after_period <- function() {
    ...
    t1 <- nanotime(1:100 * one_second_duration * 2 + one_second_duration)
    t2 <- nanotime(1:10 * one_second_duration * 10)
    checkEquals(align.idx(t1, t2, end=one_second_period, eopen=FALSE, tz="America/New_York"), seq(5, 50, 5))
}
...
test_align_none.equal <- function() {
    ...
    setkey(dt1, index)
    t2 <- nanotime(1:10 * one_second_duration * 10)
    checkEquals(align(dt1, t2, eopen=FALSE), dt1[1:10 * 10])
}
...
test_align_duration.after <- function() {
    ...
    setkey(dt1, index)
    t2 <- nanotime(1:10 * one_second_duration * 10)
    checkEquals(align(dt1, t2, start=as.nanoduration(0), end=one_second_duration, eopen=FALSE), dt1[seq(5, 50, 5)])
}
...
test_align_period.after <- function() {
    ...
    setkey(dt1, index)
    t2 <- nanotime(1:10 * one_second_duration * 10)
    checkEquals(align(dt1, t2, end=one_second_period, eopen=FALSE, tz="America/New_York"), dt1[seq(5, 50, 5)])
}
...
test_align_period.before <- function() {
    ...
    ## 50:
    checkEquals(align(dt1, t2, start=-as.nanoperiod("1d"), sopen=TRUE, eopen=FALSE, tz="America/New_York"), dt1[c(1, 25, 50, 74)])
}
...
test_align.func.variable_start_end_overlapping <- function() {
    ...
    checkEquals(align(dt1, t2, start=-10*one_second_duration, end=10*one_second_duration, sopen=TRUE, eopen=TRUE, func=colMeans), exp)    
}
test_align.func_missing <- function() {
    ...
    exp[,2] <- exp[,2] ^ 2
    setkey(exp, index)
    checkEquals(align(dt1, t2, end=as.nanoduration(1), eopen=FALSE, func=square_col1), exp)
}
test_align.func_error_dim <- function() {
    ...
    checkException(align(dt1, t2, end=as.nanoduration(1), func=square_col1))
}
test_align.func_error_incorrect_function <- function() {
    ...
    checkException(align(dt1, t2, end=as.nanoduration(1), func=f))
}
